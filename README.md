...
[![Release](https://img.shields.io/badge/release-v1.7.13.32-blue)](https://github.com/kgillard/oom-watchdog/releases/tag/v1.7.13.32)

---

## Download

Pre-built JARs are available in the [v1.7.13.32 release](https://github.com/kgillard/oom-watchdog/releases/tag/v1.7.13.32):

| Artefact | Description | Size |
|----------|-------------|------|
| [`oom-watchdog.jar`](https://github.com/kgillard/oom-watchdog/releases/download/v1.7.13.32/oom-watchdog.jar) | Fat JAR — monitoring agent + CLI entry point | ~157 KB |
| [`test-harness.jar`](https://github.com/kgillard/oom-watchdog/releases/download/v1.7.13.32/test-harness.jar) | Fat JAR — interactive OOM test harness | ~171 KB |
...
curl -L -o oom-watchdog.jar \
  https://github.com/kgillard/oom-watchdog/releases/download/v1.7.13.32/oom-watchdog.jar
...
| `--qradar-host <host>` | _(disabled)_ | QRadar / syslog target hostname or IP. **Recommended: use the physical interface IP of the QRadar Event Processor** (e.g. `9.60.246.81`). <strong>As of v1.7.13.24</strong>, when the host resolves to a local interface address (including loopback or the host's own NIC IP), UDP is automatically upgraded to TCP. <strong>As of v1.7.13.32</strong>, the TCP connection is directed at the machine's real NIC IP (not loopback), because QRadar's ecs syslog listener binds to the physical NIC address. Use `--qradar-tcp` to suppress the startup warning. |
...
| `--silent` | _(off)_ | Suppress all console/stderr output and the startup banner. Log records are still written to `--log-file`. Use when running as a background service (`nohup`, `systemd`, `cron`). |
...
> **Same-host deployments (v1.7.13.32+):** When `--qradar-host` resolves to a local interface
...
> OOM Watchdog detects this automatically and **upgrades to TCP**, and additionally **redirects
> the connection to the machine's real NIC IP** (e.g. `9.60.246.81`) rather than connecting to
> `127.0.0.1:514`, because QRadar's `ecs` syslog listener binds to the physical NIC, not the
> loopback interface. A `WARNING` is logged at startup showing the resolved NIC address;
> pass `--qradar-tcp` to suppress it. To avoid the redirect entirely, pass
> `--qradar-host <real-ip>` directly.
...
If OOM Watchdog is running and has raised at least one `WARNING` or higher alert, you will see matching events. Each event's **Event Name** column shows the LEEF Event ID (`OOM_WARNING`, `OOM_CRITICAL`, or `OOM_FIRING`), and the payload columns show the extracted heap and GC attributes.
...
3. Add the condition:
   ...
   and when the Event Name contains "OOM_CRITICAL" or "OOM_FIRING"
   ```
...
| LEEF Version | `1.1` |
| Event IDs | `OOM_OK`, `OOM_WARNING`, `OOM_CRITICAL`, `OOM_FIRING` |
