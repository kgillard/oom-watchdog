...
# Destination file to append configuration blocks into
OUTPUT_FILE="/etc/targets.conf"
SOURCE_DIR="/etc/default"
...
for filepath in "$SOURCE_DIR"/*.env; do
  ...
  if [[ -n "$jmxport" ]]; then
    # Extracts just the service name (e.g., /etc/defaults/arc_builder.env -> arc_builder)
